...
int Req(char *mess)
{
	...
	if (rtEZRequest(mess, LocaleMsg[CA_YES_NO], NULL, (struct TagItem *)tags))
		return (1);
	else
		return (0);
}
...
int Freq(char *file, int mode, char *text)
{
	...
	if (mode != 2)
		rtChangeReqAttr(req, RTFI_Dir, path, TAG_DONE);
	if (mode == 0)
		ret = (APTR)rtFileRequest(req, file, text, TAG_DONE);
	if (mode == 1)
		ret = (APTR)rtFileRequest(req, NULL, text, RTFI_Flags, FREQF_NOFILES, TAG_DONE);
	if (mode == 2)
		ret = (APTR)rtFileRequest(req, NULL, text, RTFI_VolumeRequest, VREQF_ALLDISKS | VREQF_NOASSIGNS, TAG_DONE);
